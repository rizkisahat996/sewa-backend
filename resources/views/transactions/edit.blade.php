...
@section('content')
    <div class="container">
        ...
        <form class="grid lg:grid-cols-3 gap-6" action="{{ route('transactions.update', $transaction->id) }}" method="POST" id="transaction-form">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <select name="renter_id" id="renter_id" class="form-select @error('renter_id') is-invalid @enderror" required>
                    <option value="" selected disabled>Choose...</option>
            ...
            <div>
                ...
                <input type="datetime-local" name="start_date" id="start_date"
                    ...
                    value="{{ (new DateTime($transaction->start_date))->format('Y-m-d\TH:i') }}" required>
                @error('start_date')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div>
                ...
                <input type="datetime-local" name="end_date" id="end_date"
                    ...
                    value="{{ (new DateTime($transaction->end_date))->format('Y-m-d\TH:i') }}" required>
                @error('end_date')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div>
                ...
                <input type="number" name="total_amount" id="total_amount"
                    class="form-input @error('total_amount') is-invalid @enderror"
                    value="{{ $transaction->total_amount }}" step="0.01" required readonly>
                @error('total_amount')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div class="col-span-3">
                <button class="btn bg-primary text-white" type="submit">Update Transaction</button>
            </div>
        </form>
    </div>
